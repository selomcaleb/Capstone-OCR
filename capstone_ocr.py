...
def parse_results(text):
    ...
    # Extracting candidate name and examination centre using generalized patterns
    ...
    for part in parts:
        ...
        if re.match(r"^[A-Z]", clean_part) and not re.match(r"^(A1|B2|C4|D7|E8|F9)$", clean_part) and not re.match(r"^(EXCELLENT|VERY GOOD|GOOD|CREDIT|PASS|FAIL)$", clean_part):
            subjects.append(clean_part.replace(' ', '_').replace('(', '').replace(')', '').lower())
        elif re.match(r"^(A1|B2|C4|D7|E8|F9)$", clean_part):
...
def assign_score(aggregate):
    if 6 <= aggregate <= 9:
        ...
        return 0  # Return 0 if the aggregate doesn't fall within any of the defined ranges
